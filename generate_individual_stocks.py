...
# Paths
data_etfs_path = Path(__file__).parent / 'data' / 'etfs'
...
# Process only ETFs
csv_files = sorted(data_etfs_path.glob('*.csv'))

symbol_list = []

print(f"Processing {len(csv_files)} ETF files...")
...
print(f"\n✓ Created {len(symbol_list)} individual ETF JSON files")
